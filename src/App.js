...
class App extends React.Component{
  constructor(){
    ...
    //this.calcOutline=this.calcOutline.bind(this)
  ...
  calcOutline=()=>{
    //var outline=[{cName:"outline", key:100, x1:1, y1:1, x2:211, y2:231}];
    var outline=[];
    var points=this.calcItems();
    var points=this.calcItems();//this.state.items?this.state.items:
    for(let i=1; i<points.length; i+=1){
      var Tx2=points[i].x;
      var Ty2=points[i].y;
      var Tx1=points[i-1].x;
      var Ty1=points[i-1].y;
      outline.push({cName:"outline", key:i+1000, x1:Tx1, y1:Ty1, x2:Tx2, y2:Ty2});
    }
    //outline.push(<line className="outline" key={21*132} x1={points[0].x} y1={points[0].y} x2={points[points.length-1].x} y2={points[points.length-1].y}/>)
    if(points.length)
  ...
  addText=()=>{
    ...
    for(let i=0; i<points.length; i+=1){
      ...
      outline.push({cName:"label",text:this.state.objects[i], key:i+3000, x1:Tx1, y1:Ty1});
    }
    //outline.push(<line className="outline" key={21*132} x1={points[0].x} y1={points[0].y} x2={points[points.length-1].x} y2={points[points.length-1].y}/>)
    //outline.push({cName:"outline", key:9999999, x1:points[0].x,y1:points[0].y, x2:points[points.length-1].x, y2:points[points.length-1].y});
    //console.log(outline);
